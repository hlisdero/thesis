...
\begin{definition}[Liveness]
    A Patri net $(N, M_0)$ is said to be \textit{live}
    (or equivalently $M_0$ is said to be a \textit{live marking} for N) if,
    for every marking reachable from $M_0$, it is possible to fire \textbf{any} transition
    of the net by progressing through some firing sequence.
\end{definition}

When a net is live, it can always continue executing,
no matter which specific transitions fired before.
...
\begin{definition}[Deadlock in Petri nets]
...
A transition is \textit{live} if it is not deadlocked.
