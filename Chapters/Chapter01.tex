...
Two enabled transitions are said to be in \textit{conflict}
...
In this sense, the Petri net model adopts an \textit{interleaved model of parallelism}, that is,
...
This set is called the \textit{reachability set}.
...
Having introduced the reachability set $R(N, M_0)$ in Sec. \ref{sec:reachability}, we can now present the major
analysis technique, which has been used with Petri nets: the \textit{reachability tree}.
