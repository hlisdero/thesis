...
This is seen in the initial place \uppercase{P1} of Fig. \ref{fig:state-machine-example}, where the user must select which coin to insert.
...
\begin{definition}[Concurrency in Petri nets]
...
and one outgoing arc. This subclass of Petri nets allows the representation of concurrency but not decisions (conflicts).

\begin{definition}[Marked graphs]
...
Each token represents a process and the choice of $t_1$ or $t_2$
represents whether the process performs a read or a write operation.

It makes use of weighted edges to remove atomically
$k - 1$ tokens from $p_3$ before performing a write (transition $t_2$),
...
but when one process is reading, no process is allowed to read, that is $p_2$ will be empty.
It can be easily verified that the mutual exclusion property is satisfied for this model.
