...
Branching as in Rust's \texttt{match} or \texttt{if} statements can occur only in terminators.
...
as simple conditional or unconditional \texttt{branch} instructions.
...
It should be noted that the function call to \mintinline{Rust}{std::env::args().len()}
...
\begin{itemize}
    ...
    \item \texttt{RETURN:} returns from the function.
          The return value is always stored in the local variable \texttt{\_0}, as we will see shortly.
...
and are emitted by the compiler as a string of the form ``\texttt{\_<index>}''.
In particular, the return value of the function
is always stored in the first local \texttt{\_0}.
...
\begin{itemize}
    \item The \mintinline{Rust}{main()} function starts at BB0.
    \item A function is called (\mintinline{Rust}{std::env::args()})
          to obtain an iterator over the arguments provided to the program.
    \item The return value of the function, the iterator,
          is assigned to the local \texttt{\_3}.
...
\begin{itemize}
    \item A reference to the iterator stored in \texttt{\_3} is generated
          and stored in the local \texttt{\_2} (similar to the ``\&'' operator in C).
          This is necessary for calling methods
          because methods receive a reference
          to a struct of the same type (\mintinline{Rust}{&self}) as their first argument.
    \item The reference stored in \texttt{\_2} is passed to
          the method \mintinline{Rust}{std::env::Args::len()} by moving
          and the function is called.
    \item The return value of the function,
          the number of arguments passed to the function, is assigned to the local \texttt{\_1}.
...
\begin{itemize}
    \item The variable \texttt{\_3}, whose value is the iterator over the arguments,
...
\begin{itemize}
    \item The function returns.
          The return value (local \texttt{\_0}) is of type "unit"\footnote{\url{https://doc.rust-lang.org/std/primitive.unit.html}},
          which is similar to a \texttt{void} function in C, i.e. it does not return anything.
          This is how \mintinline{Rust}{main()} was defined in Listing \ref{lst:rust-code-example}.
\end{itemize}
