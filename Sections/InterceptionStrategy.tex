...
In this section, a rationale for selecting the \acrshort{MIR} representation
...
By building on top of the existing \acrshort{MIR} representation,
...
Repurposing the \acrshort{MIR} representation means our tool can benefit from these updates
...
Furthermore, as it will be explained in the next section,
the \acrshort{MIR} is a form of \acrfull{CFG}, i.e. a type of graph found in compilers.
...
Therefore, it becomes easier to convert the \acrshort{MIR} representation to a Petri net,
...
Incremental compilation was in fact one of the reasons
why \acrshort{MIR} was introduced in the first place \cite{matsakis2016mir}.
...
on the \acrshort{MIR} representation.

The most important one is that the \acrshort{MIR} representation is subject to change.
No stability guarantees are made in terms of
how the Rust code will be translated to \acrshort{MIR}
or which constituent elements \acrshort{MIR} has.
...
the \acrshort{MIR} representation undergoes modifications
...
They were always rather small and detected by tests.
We will discuss how tests play a significant role
in coping with these changes in Sec. \ref{sec:integration-tests}.

On the same note, \cite{meyer2020} also relied on the \acrshort{MIR} representation
...
Similarly, some parts of the \acrfull{MIR} representation may be superfluous
...
These benefits outweigh the cons and make the \acrshort{MIR} representation
